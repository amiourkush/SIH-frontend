...
export default function GoogleLogin() {
  ...
  const [accountEmail] = useState("vaishnavi.railops@gmail.com");
          ...
          <div>
            ...
            <div className="flex items-center gap-3 mb-6">
              ...
              <div>
                ...
                <p className="text-[11px] font-mono text-slate-400 tracking-widest uppercase">GOOGLE SSO GATEWAY</p>
              </div>
            </div>
            ...
            <p className="text-slate-300 text-xs sm:text-sm leading-relaxed mb-6">
              Authenticate via OAuth 2.0 using your government-linked Google email address.
            </p>
            ...
            <div className="bg-[#0b2042]/90 border border-blue-900/60 rounded-xl p-4 space-y-3">
              ...
              <div className="text-xs font-mono text-slate-300 space-y-1">
                <div>Account: <strong className="text-white">{accountEmail}</strong></div>
                <div>Domain: <strong className="text-blue-300">Google Verified Railway Ops</strong></div>
                <div>Status: <strong className="text-emerald-400">Security Clearance Verified</strong></div>
              </div>
            </div>
          </div>
        ...
        <div className="lg:col-span-7 bg-white rounded-2xl p-6 sm:p-10 shadow-xl border border-slate-200/80 flex flex-col justify-between">
          <div>
            <div className="flex items-start justify-between gap-4 mb-6">
              <div>
                ...
                <p className="text-xs text-slate-500 mt-1">
                  Confirm your Google identity to proceed to the RailETA AI Console.
                </p>
              </div>
            ...
            <form onSubmit={handleVerify} className="space-y-6">
              ...
              <div className="border border-slate-300 rounded-xl p-4 flex items-center justify-between bg-slate-50 hover:bg-slate-100/80 transition cursor-pointer">
                <div className="flex items-center gap-3">
                  <div className="h-10 w-10 rounded-full bg-blue-600 text-white font-bold flex items-center justify-center text-base shadow">
                    V
                  </div>
                  <div>
                    <div className="text-sm font-bold text-slate-900">Vaishnavi (Railway Operations)</div>
                    <div className="text-xs font-mono text-slate-500">{accountEmail}</div>
                  </div>
                </div>
                <CheckCircle2 className="h-5 w-5 text-emerald-600 shrink-0" />
              </div>

              <div className="bg-blue-50 border border-blue-200 rounded-xl p-3.5 flex items-center gap-2.5 text-xs text-blue-900 font-medium">
                <ShieldCheck className="h-4 w-4 text-blue-600 shrink-0" />
                <span>Google OAuth Token pre-validated for test environment. Click below to continue.</span>
              </div>

              <button
                type="submit"
                disabled={isVerifying}
                className="w-full bg-[#061630] hover:bg-[#0b2247] text-white rounded-xl py-3.5 px-4 font-bold text-sm shadow-lg flex items-center justify-center gap-2 transition cursor-pointer"
              >
                {isVerifying ? (
                  <span>Authenticating Google Identity...</span>
                ) : (
                  <>
